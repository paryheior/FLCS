...
class DIFF(nn.Module):
    ...
    def build(self,
        ...
        print(f'item embedding dim: {emb_dim}, side information dim: {self.output_emb_size}')
...
#         )
#         # 初始化权重
#         for m in self.mean_encoder:
#             if isinstance(m, nn.Linear):
#                 nn.init.xavier_normal_(m.weight)
#                 nn.init.constant_(m.bias, 0)

#         for m in self.mean_encoder_p:
#             if isinstance(m, nn.Linear):
#                 nn.init.xavier_normal_(m.weight)
#                 nn.init.constant_(m.bias, 0)
        return
    ...
    def warm_item_id(self, x_dict):
        ...
        for item_f in self.item_features: 
            ...
            if type == 'spr':
                ...
                # emb = self.warmup_emb_layer[name](x).mean(dim=1, keepdim=True).squeeze()
            else:
                raise ValueError('illegal feature tpye for warm: {}'.format(item_f))
            item_embs.append(emb)
